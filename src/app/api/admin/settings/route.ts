...
export async function PUT(request: NextRequest) {
  ...
  let value = String(body.value ?? "").slice(0, 500);
  ...
  // L6: announcement.link render thành <Link href> toàn site — chỉ cho
  // path nội bộ hoặc https, chặn javascript:/data:.
  if (key === "announcement.link") {
    const v = value.trim();
    if (v && !v.startsWith("/") && !v.startsWith("https://")) {
      return NextResponse.json({ error: "Link phải là đường dẫn nội bộ (/) hoặc https://." }, { status: 422 });
    }
    value = v || "/";
  }
